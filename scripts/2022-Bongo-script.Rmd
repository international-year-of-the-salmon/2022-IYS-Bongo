...
Next, create the occurrence extensions:

```{r}
IYS2022_occ <- IYS2022_occ %>%
  select(Taxon, sex = Sex, 
         lifeStage = Dev_stage, 
         verbatimIdentification = original_id, eventID, Abundance_m3) %>%
...
IYS2022_occ_final <- IYS2022_occ_final %>%
  ...
  mutate(scientificName = ifelse(!is.na(scientificNameAuthorship), 
                                 ...
                                 scientificName)) %>%
  select(-Taxon, -Abundance_m3, -AphiaID)

IYS2022_occ_final[is.na(IYS2022_occ_final)] <- ""
write_csv(IYS2022_occ_final, here("standardized_data", "IYS2022_zoop_occ_final.csv"))
```

Run some checks on the event and occurrence data tables:
...
# Plot points on a map to ensure no observations on land:
obistools::plot_map(IYS2022_bongo_event, zoom = TRUE)
obistools::check_onland(unlist(lapply(IYS2022_bongo_event, as.numeric)), report = T)
...
obistools::report(unlist(IYS2022_bongo_event))
...
tinro2022_bongo_occ$identificationQualifier <- ifelse(grepl("\\b sp.\\b|\\b spp.\\b|\\b sp. \\b", tinro2022_bongo_occ$verbatimIdentification), "sp. indet.", NA)
tinro2022_bongo_occ$identificationQualifier <- ifelse(tinro2022_bongo_occ$verbatimIdentification == "Clausocalanus sp. - really small, 0.5mm  tiny 5th leg AF voucher", "sp. indet.", tinro2022_bongo_occ$identificationQualifier)
tinro2022_bongo_occ$identificationQualifier <- ifelse(tinro2022_bongo_occ$verbatimIdentification == "Clausocalanus sp. , 1.5mm  tiny 5th leg AF voucher", "sp. indet.", tinro2022_bongo_occ$identificationQualifier)
...
IYS2022_bongo_final <- IYS2022_bongo_final %>%
  mutate(lifeStage = ifelse(grepl("bract|BRACTS", verbatimIdentification), "polygastric", lifeStage),
...
#TODO: Remove following lines once crosswalk is complete: 
IYS2022_bongo_final[is.na(IYS2022_bongo_final)] <- ""
write_csv(IYS2022_bongo_final, here("standardized_data", "IYS2022_bongo_final.csv"))
```

On November 17, 2023, Joanne Breckenridge sent 2022 standardized files. Making sure the contents are similar:
