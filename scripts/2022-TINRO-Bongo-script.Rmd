...
nw2022_bongo_metadata <- rbind(nw2022_bongo_metadata_2, nw2022_bongo_metadata_1) 
...
nw2022_bongo_occ <- nw2022_bongo_event %>%
  mutate(occurrenceID = paste(eventID, row_number(), sep = "-"),
         occurrenceStatus = "present",
         preparations = "Formalin")

shim2022_bongo_occ <- shim2022_bongo_event %>%
  mutate(occurrenceID = paste(eventID, row_number(), sep = "-"),
         occurrenceStatus = "present", 
         preparations = "Formalin")
...
# Standardize data to Darwin Core:
shim_nw_bongo_occ <- shim_nw_bongo_occ %>%
  rename(verbatimIdentification = species_recorded)

# From the scientific_name column, remove 'SP.' and 'SP.' to begin with:
shim_nw_bongo_occ$scientific_name <- gsub("\\b SP.\\b|\\b SP.\\b|\\b SP\\b", "", shim_nw_bongo_occ$scientific_name) 
...
shim_nw_bongo_occ$scientific_name <- gsub(" LARVAE| JUVENILES| FURCILIA| NURSE| MEDIUM| SMALL| EXTRA SMALL| NAUPLII| ZOEA| CALYPTOPIS|
                                          AF| AM| V$| IV| III| II| I$", "", shim_nw_bongo_occ$scientific_name)

manual_check <- shim_nw_bongo_occ %>% select(scientific_name, verbatimIdentification) %>% distinct()
...
#TODO: 7 names have to be changed to match WoRMS accepted names. These have to be changed in the original data table
...
shim_nw_bongo_worms_id <- filter(shim_nw_bongo_worms_id, !(AphiaID %in% c("534090", "254409", "999078")))
...
Next, the Bongo data collected by the TINRO: 
